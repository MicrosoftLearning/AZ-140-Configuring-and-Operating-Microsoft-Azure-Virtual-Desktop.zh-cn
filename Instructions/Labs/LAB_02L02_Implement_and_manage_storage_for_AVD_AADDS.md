...
- 已完成的实验室**准备部署 Azure 虚拟桌面 (Microsoft Entra DS)**
...
1. 从实验室计算机上的 Azure 门户中，搜索并选择“虚拟机”****，然后从“虚拟机”**** 边栏选项卡中选择“az140-cl-vm11a”**** 条目。 这将打开“az140-cl-vm11a”**** 边栏选项卡。
...
1. 在“创建存储帐户”边栏选项卡的“基本信息”选项卡上，指定以下设置（其他设置保留默认值） ：
   ...
   >**备注**：确保存储帐户名称的长度不超过 15 个字符。 该名称将用于在 Active Directory 域服务 (AD DS) 域中创建计算机帐户，该帐户与包含存储帐户的 Azure 订阅关联的 Microsoft Entra 租户集成。 这将允许在访问此存储帐户中托管的文件共享时进行基于 AD DS 的身份验证。
...
1. 在“新建文件共享”**** 边栏选项卡上，指定以下设置，然后选择“创建”****（将其他设置保留为默认值）：
