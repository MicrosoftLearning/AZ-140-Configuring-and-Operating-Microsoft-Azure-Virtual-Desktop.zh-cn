...
1. 在 Azure 门户中 Cloud Shell**** 的 PowerShell 会话中，运行以下命令以设置下个命令的位置（将 `<Azure_region>` 占位符替换为要用于此实验室的 Azure 区域的名称，例如 `eastus`）：

   ```powershell
   $location = '<Azure_region>'
   ```

1. 在 Azure 门户中 Cloud Shell**** 的 PowerShell 会话中，运行以下命令以确定 vCPU 的当前使用情况以及 Azure VM StandardDSv3Family**** 和 StandardBSFamily**** 的相应限制： 
...
1. 查看上一步中所执行命令的输出，并确保目标 Azure 区域中 Azure VM 的“标准 DSv3 系列 vCPU”**** 中至少有 30**** 个可用的 vCPU。 如果已满足此条件，请直接进行下一个练习。 否则，继续本练习的下一个任务。 
...
1. 在“Azure Pass - 赞助 | 使用情况 + 配额”**** 边栏选项卡上，选择“区域”****，在下拉列表中选中要用于此实验室的 Azure 区域名称旁边的复选框，选择“应用”****，确保“计算”**** 条目出现在“区域”**** 条目左侧的下拉列表中，然后在搜索框中键入“Standard DSv3”****。 
1. 在结果列表中，选中“标准 DSv3 系列 vCPU”**** 条目旁边的复选框，选择工具栏中的“请求配额增加”**** 条目，然后在下拉列表中选择“输入新限制”****。
...
1. 允许配额请求完成。  片刻后，“配额详细信息”边栏选项卡将指定已批准请求并增加配额。 关闭“配额详细信息”边栏选项卡。
   ...
   >**注意**：根据 Azure 区域的选择和当前需求，可能需要提出支持请求。 有关创建支持请求的过程的说明，请参见[创建 Azure 支持请求](https://docs.microsoft.com/en-us/azure/azure-portal/supportability/how-to-create-azure-support-request)。
...
1. 创建将同步到 Microsoft Entra 的 AD DS 用户和组
1. 配置 AD DS UPN 后缀
...
1. 从“管理员: Windows PowerShell ISE”**** 控制台中运行以下命令，以创建一个 AD DS 组织单位，该组织单位将包含同步到本实验室中使用的 Microsoft Entra 租户的范围内的对象：
...
1. 在“管理员: Windows PowerShell ISE”**** 脚本窗格中运行以下命令，以创建 AD DS 用户帐户，这些帐户将同步到本实验室中使用的 Microsoft Entra 租户（将 `<password>` 占位符替换为随机的复杂密码）：

   > **备注**：确保记录所使用的密码。 稍后要在本实验室和后续实验室中用到它们。
...
1. 从“管理员: Windows PowerShell ISE”**** 控制台中运行以下命令，以检索与 Azure 订阅关联的 Microsoft Entra 租户的 ID 属性：
...
1. 从“管理员: Windows PowerShell ISE”**** 控制台中运行以下命令，以对 Microsoft Entra 租户进行身份验证：

   ```powershell
   Connect-AzureAD -TenantId $tenantId
   ```

1. 如果出现提示，请使用你在此任务中用到的相同凭据（在此实验室中使用的订阅中具有所有者角色的用户帐户）登录。 
1. 从“管理员: Windows PowerShell ISE”**** 控制台中运行以下命令，以检索与 Azure 订阅关联的 Microsoft Entra 租户的主 DNS 域名：

   ```powershell
   $aadDomainName = ((Get-AzureAdTenantDetail).VerifiedDomains)[0].Name
   ```

1. 从“管理员: Windows PowerShell ISE”**** 控制台中运行以下命令，以将与 Azure 订阅关联的 Microsoft Entra 租户的主 DNS 域名添加到 AD DS 林的 UPN 后缀列表中：

   ```powershell
   Get-ADForest|Set-ADForest -UPNSuffixes @{add="$aadDomainName"}
   ```

1. 从“管理员: Windows PowerShell ISE”**** 脚本窗格中运行以下命令，以将与 Azure 订阅关联的 Microsoft Entra 租户的主 DNS 域名分配为 AD DS 域中所有用户的 UPN 后缀：
...
1. 从“管理员: Windows PowerShell ISE”**** 脚本窗格中运行以下命令，以将全局管理员角色分配给新创建的 Microsoft Entra 用户： 
...
1. 从“管理员: Windows PowerShell ISE”**** 脚本窗格中运行以下命令，以标识新创建的 Microsoft Entra 用户的用户主体名称：
