...
# <a name="lab---package-azure-virtual-desktop-applications-ad-ds"></a>实验室 - 打包 Azure 虚拟桌面应用程序 (AD DS)
...
### <a name="exercise-1-prepare-for-and-create-msix-app-packages"></a>练习 1：准备并创建 MSIX 应用程序包
...
#### <a name="task-1-prepare-for-configuration-of-azure-virtual-desktop-session-hosts"></a>任务 1：准备配置 Azure 虚拟桌面会话主机
...
1. 从“Cloud Shell”窗格中的 PowerShell 会话运行以下命令，以启动你将在本实验室中使用的 Azure 虚拟桌面会话主机 Azure VM：
   ...
   >**注意**：无需等待 Azure VM 启动，直接继续执行下一个任务。

#### <a name="task-2-deploy-an-azure-vm-running-windows-10-by-using-an-azure-resource-manager-quickstart-template"></a>任务 2：使用 Azure 资源管理器快速启动模板部署运行 Windows 10 的 Azure VM
...
#### <a name="task-3-prepare-the-azure-vm-running-windows-10-for-msix-packaging"></a>任务 3：为 MSIX 打包准备运行 Windows 10 的 Azure VM
...
1. 系统提示时，使用 wvdadmin1 用户名和在创建此用户帐户时设置的密码登录。 
1. 在与 az140-cl-vm42 的远程桌面会话中，以管理员身份启动 Windows PowerShell ISE，并从“管理员 :Windows PowerShell ISE”控制台中运行以下命令，为 MSIX 打包准备操作系统：
   ...
   > **注意**：上面的最后一个注册表更改会禁用用户访问控制。 这在技术上不是必需的，但可以简化本实验室中说明的过程。

#### <a name="task-4-generate-a-signing-certificate"></a>任务 4：生成签名证书

> **注意**：在本实验室中，你将使用自签名证书。 在生产环境中，应使用由公共证书颁发机构或内部证书颁发机构颁发的证书，具体取决于预期用途。
...
1. 在“管理员:Windows PowerShell ISE”控制台中运行以下命令，以启动面向本地计算机证书存储的“证书”控制台：
...
1. 在“证书导出向导”的“正在完成证书导出向导”页上，选择“完成”，然后选择“确定”，确认已成功导出   。 
   ...
   > **注意**：由于你将使用自签名证书，因此需要在目标会话主机上的“受信任的用户”证书存储中安装该证书。

1. 在“管理员:Windows PowerShell ISE”控制台中运行以下命令，以在目标会话主机上的“受信任的用户”证书存储中安装新生成的证书：
...
#### <a name="task-5-download-software-to-package"></a>任务 5：下载要打包的软件
...
#### <a name="task-6-install-the-msix-packaging-tool"></a>任务 6：安装 MSIX 打包工具
...
#### <a name="task-7-create-an-msix-package"></a>任务 7：创建 MSIX 包
...
1. 在“创建新包”向导的“准备计算机”页上查看建议 。 如果有挂起的重启，请重启操作系统，使用帐户 ADATUM\wvdadmin1 重新登录，然后先重启 MSIX 打包工具再继续 。 

   >**注意**：MSIX 打包工具会暂时禁用 Windows 更新和 Windows Search。 在本例中，Windows Search 服务已处于禁用状态。 
...
### <a name="exercise-2-implement-an-msix-app-attach-image-for-azure-virtual-desktop-in-azure-ad-ds-environment"></a>练习 2：在 Azure AD DS 环境中实现 Azure 虚拟桌面的 MSIX 应用附加映像
...
#### <a name="task-1-enable-hyper-v-on-the-azure-vms-running-window-10-enterprise-edition"></a>任务 1：在运行 Windows 10 企业版的 Azure VM 上启用 Hyper-V
...
1. Hyper-V 组件安装完成后，键入“是”重启操作系统。 重启后，使用帐户 wvdadmin1 和密码 Pa55w.rd1234 重新登录 。

#### <a name="task-2-create-an-msix-app-attach-image"></a>任务 2：创建 MSIX 应用附加映像

1. 在与 az140-cl-vm42 的远程桌面会话中，启动 Microsoft Edge，浏览到 https://aka.ms/msixmgr  。 这会自动将 msixmgr.zip 文件（MSIX mgr 工具存档）下载到 Downloads 文件夹中 。
...
1. 在“管理员:Windows PowerShell ISE”脚本窗格中运行以下命令，以安装新创建的 VHD 文件：

   ```powershell
   $vhdObject = Mount-VHD -Path 'C:\Allfiles\Labs\04\MSIXVhds\XmlNotepad.vhd' -Passthru
   ```

1. 在“管理员:Windows PowerShell ISE”脚本窗格中运行以下命令，以初始化磁盘、创建新分区、格式化该分区，并为该分区分配第一个可用的驱动器号：
   ...
   > **注意**：如果出现提示格式化 F: 驱动器的弹出窗口，请选择“取消”。

1. 在“管理员:Windows PowerShell ISE”脚本窗格中运行以下命令，以创建将托管 MSIX 文件的文件夹结构，并将你在上一个任务中创建的 MSIX 包解压缩到其中：
...
### <a name="exercise-3-implement-msix-app-attach-on-azure-virtual-desktop-session-hosts"></a>练习 3：在 Azure 虚拟桌面上实现 MSIX 应用附加
...
#### <a name="task-1-configure-active-directory-groups-containing-azure-virtual-desktop-hosts"></a>任务 1：配置包含 Azure 虚拟桌面主机的 Active Directory 组
...
1. 在“管理员:Windows PowerShell ISE”脚本窗格中运行以下命令，创建将同步到本实验室中所用 Azure AD 租户的 AD DS 组对象：
   ...
   > **注意**：你将使用此组向 Azure 虚拟桌面主机授予对 az140-42-msixvhds 文件共享的权限。
...
1. 在“管理员:Windows PowerShell ISE”脚本窗格中运行以下命令，以重启属于“az140-hosts-42-p1”组成员的服务器：
...
1. 在“Microsoft Azure Active Directory Connect”窗口的“连接到 Azure AD”页上，使用在本任务稍早部分标识的用户帐户“aadsyncuser”的用户主体名称和在创建此用户帐户时设置的密码进行身份验证  。
...
#### <a name="task-2-set-up-the-azure-files-share-for-msix-app-attach"></a>任务 2：为 MSIX 应用附加设置 Azure 文件存储共享
...
1. 在与“az140-cl-vm42”的远程桌面会话中，启动 Microsoft Edge，在 InPrivate 模式下导航到 [Azure 门户](https://portal.azure.com)，然后通过提供具有本实验室所用订阅的所有者角色的用户帐户凭据登录。
   ...
   > **注意**：请确保使用 Microsoft Edge InPrivate 模式。

1. 在与 az140-cl-vm42 的远程桌面会话中，在显示 Azure 门户的 Microsoft Edge 窗口中搜索并选择“存储帐户”，然后在“存储帐户”边栏选项卡中选择配置为托管用户配置文件的存储帐户。

   > **注意**：实验室的这一部分视实验室“Azure 虚拟桌面配置文件管理 (AD DS)”或“Azure 虚拟桌面配置文件管理 (Azure AD DS)”的完成情况而定 

   > **注意**：在生产场景中，应考虑使用单独的存储帐户。 这需要为该存储帐户配置 Azure AD DS 身份验证，你已经为托管用户配置文件的存储帐户实现了该身份验证。 你将使用同一存储帐户，以尽可能避免在各实验室中执行重复的步骤。
...
1. 在“添加角色分配”边栏选项卡上，指定以下设置并选择“保存” ：
   ...
   > **注意**：az140-wvd-admins 组包含将用于配置共享权限的 wvdadmin1 用户帐户 。 
...
1. 重复前两个步骤以配置以下角色分配：
   ...
   > **注意**：Azure 虚拟桌面用户和主机至少需要具有对文件共享的读取访问权限。
...
1. 在与 az140-cl-vm42 的远程桌面会话中，在“命令提示符”窗口中运行以下命令，以向会话主机的计算机帐户授予所需的 NTFS 权限 ：
   ...
   >**注意**：你还可以在以 ADATUM\\wvdadmin1 身份登录时使用文件资源管理器设置这些权限。 

   >**注意**：接下来你将验证 MSIX 应用附加的功能
...
#### <a name="task-3-mount-and-register-the-msix-app-attach-image-on-azure-virtual-desktop-session-hosts"></a>任务 3：在 Azure 虚拟桌面会话主机上装载和注册 MSIX 应用附加映像
...
#### <a name="task-4-publish-msix-apps-to-an-application-group"></a>任务 4：将 MSIX 应用发布到应用程序组

> **注意**：你需要将 MSIX 应用发布到远程应用和桌面应用组。 
...
#### <a name="task-5-validate-the-functionality-of-msix-app-attach"></a>任务 5：验证 MSIX 应用附加的功能
...
### <a name="exercise-4-stop-and-deallocate-azure-vms-provisioned-and-used-in-the-lab"></a>练习 4：停止并解除分配在实验室中预配和使用的 Azure VM
